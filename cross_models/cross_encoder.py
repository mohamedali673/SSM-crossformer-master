...
class SegMerging(nn.Module):
    def __init__(self, d_model, win_size, norm_layer=nn.LayerNorm):
        ...
        self.mlp1 = nn.Sequential(
            nn.Linear(d_model, d_model * win_size),
            nn.ReLU(inplace=True),
            nn.Linear(d_model * win_size, d_model),
            nn.Linear(d_model, d_model * win_size),
            nn.Linear(d_model * win_size, d_model)
        )
    ...
    def inverse(self, x):
        """
        Inverse operation of forward method.
        """
        # Ajout de la MLP avant la normalisation
        x = self.mlp1(x)
        x = self.mlp2(x)

        # Perform the inverse operation of linear transformation
        x = self.linear_trans.inverse(x)
        x = self.norm.inverse(x)

        # Perform the inverse operation of concatenation
        segs = torch.split(x, self.d_model, dim=-1)
        x = torch.cat([seg.squeeze(dim=-1) for seg in segs], dim=-2)
